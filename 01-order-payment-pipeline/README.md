...
처음에는 주문 기록(receipt)과 PaymentInfo 저장을 모두 Toss 승인 이후로 미루는 방식도 검토했습니다. 하지만 이 구조에서는 Toss 승인은 성공했는데 그 직후 서버가 내려가거나 후속 DB 저장이 실패하면, 사용자는 결제했지만 우리 DB에는 주문서도 결제 정보도 없는 블라인드 상태가 생길 수 있었습니다.

그래서 주문 기록(receipt)은 Toss 승인 전에 먼저 남기고, PaymentInfo는 Toss 승인 성공 후에만 생성하는 구조를 선택했습니다. 이때 PaymentInfo를 결제 시도나 PENDING 상태 표현에 쓰지 않고, 로컬 DB 안에서는 **`PaymentInfo가 있으면 확정 결제`로만 해석되게 유지**했습니다. 만약 Toss에는 결제가 있는데 PaymentInfo가 없는 orphan이 생기면, 이는 로컬 불변식으로 덮지 않고 PG 대사에서 외부 불일치로 탐지해 outbox 복구 경로로 보냅니다.
...
로컬 보상 실패와 결제 취소는 분리했습니다. TX2 실패 시 receipt 하드삭제·재고 복원 보상이 실패하더라도 원 예외를 가리지 않도록 로그를 남기고, 이미 승인된 Toss 결제 전체취소는 계속 진행합니다. 즉, 주문 흔적 정리 실패는 수동 확인 대상으로 남기되, 사용자 돈이 PG에 묶이는 경로는 outbox 재시도까지 이어지게 했습니다.
...
운영 레포에서 deep-dive 범위만 발췌했습니다 (범위 외 메서드는 생략 주석으로 표시). IDE 탐색 방법은 [루트 README](../README.md#코드-탐색-ide) 참고.

| 파일 | 역할 |
|------|------|
| [`ReceiptFacade.java`](../src/main/java/be/weskey/module/member/receipt/facade/ReceiptFacade.java) | 사전검증 → TX1 → Toss 승인 → TX2 → TX3 보상 오케스트레이션 |
| [`ReceiptService.java`](../src/main/java/be/weskey/module/member/receipt/service/ReceiptService.java) | 주문 저장/보상 트랜잭션 단위 구현 |
| [`PaymentCancelOutbox.java`](../src/main/java/be/weskey/shared/toss_payment/entity/PaymentCancelOutbox.java) / [`PaymentCancelTrigger.java`](../src/main/java/be/weskey/shared/toss_payment/entity/PaymentCancelTrigger.java) / [`PaymentCancelOutboxStatus.java`](../src/main/java/be/weskey/shared/toss_payment/entity/PaymentCancelOutboxStatus.java) | 취소 실패 복구 레코드 (outbox 엔티티/트리거/상태) |
| [`PaymentCancelOutboxFacade.java`](../src/main/java/be/weskey/shared/toss_payment/facade/PaymentCancelOutboxFacade.java) | outbox 재시도 처리 흐름 |
| [`PaymentCancelOutboxScheduler.java`](../src/main/java/be/weskey/common/scheduler/PaymentCancelOutboxScheduler.java) | 10분 주기 outbox 재시도 스케줄러 |
| [`PaymentReconciliationFacade.java`](../src/main/java/be/weskey/shared/toss_payment/facade/PaymentReconciliationFacade.java) / [`PaymentReconciliationService.java`](../src/main/java/be/weskey/shared/toss_payment/service/PaymentReconciliationService.java) | Toss 거래내역 대사 — orphan 결제 탐지 |
| [`PaymentReconciliationScheduler.java`](../src/main/java/be/weskey/common/scheduler/PaymentReconciliationScheduler.java) | 매일 04:00 PG 대사 스케줄러 |
