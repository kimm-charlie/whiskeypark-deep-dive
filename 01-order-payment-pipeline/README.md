...
외부 PG(Toss) 호출을 재고 락 트랜잭션 밖으로 분리(락 점유 초 단위 → 수십 ms)하고,
분리로 잃은 자동 롤백을 **보상 트랜잭션 + outbox 재시도 + PG 대사** 3단 복구 안전망으로 보강.

## 흐름

```
사전검증 → TX1(주문서+재고차감, 짧은 커밋) → Toss 승인(트랜잭션 밖)
→ TX2(PaymentInfo 확정) → 실패 시 TX3 보상(하드삭제+재고복원) → Toss 취소 실패 시 outbox
```
...
| Toss 취소 재시도 + `ALREADY_CANCELED` 멱등 | [`TossPaymentCancelService`](../src/main/java/be/weskey/shared/toss_payment/service/TossPaymentCancelService.java) |
